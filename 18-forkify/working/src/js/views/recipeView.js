...
class RecipeView extends View {
  ...
  addServingsHandler(callbackFn) {
    this._parentEl.addEventListener("click", (e) => {
      const button = e.target.closest(".btn--update-servings");
      if (!button) return;

      const servings = Number(button.dataset.servingSize);
      if (servings > 0) callbackFn(servings);
    });
  }

  addAddBookmarkHandler(callbackFn) {
    this._parentEl.addEventListener("click", (e) => {
        ...
        <div class="recipe__info">
          ...
          <div class="recipe__info-buttons">
            <button class="btn--tiny btn--update-servings" 
              data-serving-size="${this._data.servings - 1}">
              <svg>
      ...
      <div class="recipe__ingredients">
        ...
        <ul class="recipe__ingredient-list">
          ${this._data.ingredients
            .map((ingr) => this.#generateIngredientMarkup(ingr))
            .join("")}
        </ul>
      </div>
